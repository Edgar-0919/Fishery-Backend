...
| ts_value | FLOAT | 浊度（NTU） |
| water_level | FLOAT | 水位（m） |
| **TAGS** | | |
| pond_id | BINARY(50) | 鱼塘 ID |
| device_id | BINARY(50) | 设备 ID |
| product_id | BINARY(50) | 产品 ID |
...
| pond_id | BINARY(50) | 鱼塘 ID |
| device_id | BINARY(50) | 设备 ID |
| alert_type | BINARY(50) | 告警类型 |
| alert_value | FLOAT | 异常值 |
| threshold_value | FLOAT | 阈值 |
| message | BINARY(255) | 告警消息 |
| is_read | BOOL | 是否已读 |

**alert_duration** — 告警持续追踪表
...
| alert_value | FLOAT | 异常值 |
| threshold_value | FLOAT | 阈值 |
| started_at | TIMESTAMP | 异常开始时间 |
| last_checked_at | TIMESTAMP | 最后检测时间 |
| duration_minutes | INT | 已持续分钟数 |
| status | BINARY(20) | active / escalated / resolved |
| agent_decision | BINARY(500) | Agent 决策摘要 |
| agent_action | BINARY(100) | Agent 执行的动作 |

**device_commands** — 设备指令记录表

| 字段 | 类型 | 描述 |
|------|------|------|
| ts | TIMESTAMP | 时间戳 |
| pond_id | BINARY(50) | 鱼塘 ID |
| device_id | BINARY(50) | 被控设备 ID |
| command_type | BINARY(50) | 指令类型 |
| trigger_source | BINARY(50) | agent / manual / scheduled |
| trigger_alert_id | BINARY(50) | 关联告警 ID |
| params | BINARY(500) | 指令参数 JSON |
| status | BINARY(20) | pending / executed / failed / pending_approval |
| executed_at | TIMESTAMP | 执行时间 |
| result | BINARY(500) | 执行结果 |

**device_status** — 设备上下线记录表
...
|------|------|------|
| id | BIGINT | 主键（自增） |
| pond_id | VARCHAR(50) | 鱼塘 ID |
| alert_type | VARCHAR(50) | 告警类型 |
| alert_value | DOUBLE | 异常值 |
...
**control_devices** — 设备配置表

| 字段 | 类型 | 描述 |
|------|------|------|
| id | BIGINT | 主键（自增） |
| pond_id | VARCHAR(50) | 鱼塘 ID |
| device_type | VARCHAR(50) | aerator / pump / feeder |
| device_name | VARCHAR(100) | 设备名称 |
| device_identifier | VARCHAR(100) | 硬件标识/MQTT topic |
| status | ENUM | online / offline / running |
| updated_at | DATETIME | 更新时间 |

**device_mapping** — 设备映射表（设备→鱼塘绑定）

| 字段 | 类型 | 描述 |
|------|------|------|
| id | BIGINT | 主键（自增） |
| device_id | VARCHAR(50) | 设备 ID（唯一） |
| pond_id | VARCHAR(50) | 鱼塘 ID |
| pond_name | VARCHAR(100) | 鱼塘名称 |
...
提交 PR 前请确保：
1. `python app/main.py` 启动无报错
2. 无语法错误和运行时异常
3. 数据库变更已同步到 `sql/` 目录下的脚本
